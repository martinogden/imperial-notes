...
\subsubsektion{Kinematics of point particles}\vspace*{-5pt} Vectors and vector algebra; position, velocity, and acceleration in three dimensions; polar coordinates; intrinsic coordinates and path curvature.
\subsubsektion{Newton's Laws}\vspace*{-5pt} Definition of mass, momentum, inertia, and force; Axioms, or Laws of Motion. 
\subsubsektion{Forces}\vspace*{-5pt} Gravitation; forces that constrain motion: normal force and tension; friction; forces that depend on velocity: drag forces; forces that depend on position: spring forces. 
\subsubsektion{Oscillators}\vspace*{-5pt} Simple, damped, and forced oscillators; amplitude and phase difference; resonance.
\subsubsektion{Energy}\vspace*{-5pt} Kinetic and potential energies; conservative forces; stability of and motion about fixed points; potential wells and escape; energy diagrams.
\subsubsektion{Angular Momentum}\vspace*{-5pt}Central forces; orbital equation; effective potential. Systems of (interacting) particles: Two body systems; centre of mass; moment of inertia; total momentum, angular momentum, and energy for systems; variable mass systems; torque. 

\subsubsektion{Rigid Body Motion}\vspace*{-5pt}Rigid body kinematics; continuous mass distributions; rigid body dynamics with rotation about a single axis
...
%Question: \lecturemarker{1}{5 Oct} \emph{Why are we learning mechanics?}
...
%\begin{center}
%    \begin{tikzpicture}
%      \draw [domain=0:1] plot ({2*\x}, {4*(cos(45 * \x))^40});
%      \draw [domain=3:4] plot ({2*\x}, {4*(cos(45 * \x))^40});
%      \draw [domain=1:3] plot ({2*\x}, {4*(cos(45 * \x+90))^40});
%      \draw [->] (0, 0) -- (9, 0) node [right] {$t$};
%      \draw [->] (0, -0.8) -- (0, 5) node [above] {Attention};
%      \draw [dashed] (0, 4) -- (0, 0) node [below right] {$0$};
%      \draw [dashed] (8, 4) -- (8, 0) node [below] {$50$ min};
%      \draw [dashed] (4, 4) -- (4, 0) node [below] {$25$ min};
%      \draw [dashed] (0, 4) -- (8, 4);      
%    \end{tikzpicture}
% \end{center}~
...
$r$ is the distance from the origin.
 ...
 \textbf{Last Time:} \lecturemarker{2}{5 Oct}
...
\begin{center}
  ...
  \begin{figure}[H]
  \begin{center}
  \includegraphics[width = 5cm]{right.png}	
...
\begin{itemize}
\item 2D Motion\lecturemarker{6}{5 Oct} 
\item Certain systems simplify in these coordinates	
\end{itemize}
...
\]
 ...
 \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
$\hat{r}$: Unit vector pointing in the direction of increasing $r$ along lines of constant $\theta$.
...
Inertial frame $\dfrac{d^2\vec{R}}{dt^2} = 0$.\\
...
\begin{center}
    ...
    % Coordinates system
    \draw[->,line width=1pt] (-3,0) --(4,0) node[above]{$x$};
    \draw[->,line width=1pt] (-3,0) --(-3,2) node[left]{$y$};
    
    % Coordinates system
    \draw[<->,line width=1pt] (-4,1) node[above]{$\hat{i}$}-|(-5,2) node[left]{$\hat{j}$};
    
        % Coordinates system
    \draw[->,line width=1pt] (3,2)--(3,1.4) node[right]{$\vec{g}$};

        % Coordinates system
    \draw[->,line width=1pt] (-3,0)--(-2.2,1.4) node[above]{$\vec{v} = \vec{v}_0\hat{v}$};
...
\subsubsektion{Kinetic Energy\lecturemarker{28}{22 Oct}}
...
\subsubsektion{Centre of Mass \lecturemarker{29}{22 Oct} Coordinates}
